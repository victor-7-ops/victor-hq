...
import { COLUMNS, COLUMN_DOT, COLUMN_LABELS } from "@/lib/agent-orchestrator-columns"
import { rollupSessionsByAgent, rollupForHierarchy, type AgentRollup } from "@/lib/agent-orchestrator-ownership"
...
/* ──────────────────────────────────────────────
   Skill derivation from agent tools
   ────────────────────────────────────────────── */
...
const SKILL_RULES: { tools: string[]; skill: DerivedSkill }[] = [
  {
  ...
  },
  {
    ...
    skill: {
      name: "Code Execution",
      icon: "\u26A1",
      description: "Run commands and execute code",
  ...
  },
  {
    ...
    skill: {
      name: "Memory & Context",
      icon: "\uD83E\udDE0",
      description: "Search and recall stored knowledge",
...
function deriveSkills(tools: string[]): DerivedSkill[] {
  return SKILL_RULES.filter((rule) =>
    rule.tools.some((t) => tools.includes(t))
  ).map((rule) => rule.skill)
}
...
export default function AgentDetailPage({
  ...
  const [aoRollups, setAoRollups] = useState<Map<string, AgentRollup> | null>(null)
  ...
  const loadData = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([agentsData, cronData]) => {
        ...
        // Try matching by agentId first; fall back to name-prefix matching
        let matched = cronList.filter((cr: CronJob) => cr.agentId === id)
        if (matched.length === 0) {
          matched = cronList.filter(
            (cr: CronJob) => cr.name === id || cr.name.startsWith(id + "-")
          )
        }
        setCrons(matched)
      })
  ...
  useEffect(() => {
    fetch("/api/agent-orchestrator")
      ...
      .then((data) => {
        if (!data) return
        const projects: AOProject[] = Array.isArray(data.projects) ? data.projects : []
        const sessions: AOSession[] = Array.isArray(data.sessions) ? data.sessions : []
        const owners: Record<string, string> = data.owners && typeof data.owners === "object" ? data.owners : {}
        const ownerMap = new Map(Object.entries(owners))
        setAoRollups(rollupSessionsByAgent(sessions, projects, ownerMap))
      })
      .catch(() => {})
  }, [])
  ...
  async function handleImageUpload(file: File) {
    try {
      ...
      // Sync to OpenClaw IDENTITY.md
      fetch("/api/agents/update", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId: id, profileImage: dataUrl }),
      }).catch(() => {})
    } catch {
      // silently fail
    }
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ background: "var(--bg)" }}>
      ...
      >
          ...
          <div style={{ position: "relative", flexShrink: 0 }}>
            ...
            >
              ...
              {settings.agentOverrides[agent.id]?.profileImage && (
                <button
                  onClick={() => {
                    setAgentOverride(agent.id, { profileImage: undefined })
                    fetch("/api/agents/update", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ agentId: agent.id, profileImage: null }),
                    }).catch(() => {})
                  }}
                  title="Remove photo"
                  style={{
        ...
        {/* ── Orchestrator Sessions card ── */}
        {(() => {
          if (!aoRollups) return null
          const direct = aoRollups.get(agent.id)
          const hasDirect = !!direct && direct.total > 0
          if (!hasDirect && children.length === 0) return null
          const org = children.length > 0 ? rollupForHierarchy(agent.id, allAgents, aoRollups) : direct
          ...
          return (
            <Card>
              ...
              >
                <span>Orchestrator Sessions</span>
                <Link
                  href={`/agent-orchestrator`}
                  className="focus-ring"
                  style={{ fontSize: "var(--text-caption1)", color: "var(--system-blue)", textDecoration: "none", fontWeight: "var(--weight-medium)" }}
                >
                  View board &rarr;
                </Link>
              </div>
              {!hasDirect && (!org || org.total === 0) ? (
                <div style={{ fontSize: "var(--text-footnote)", color: "var(--text-tertiary)" }}>
                  No active sessions
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {hasDirect && (
                    <div>
                      <div style={{ fontSize: "var(--text-caption2)", color: "var(--text-tertiary)", marginBottom: 6 }}>
                        Owned directly ({direct!.total})
                      </div>
                        ...
                        {COLUMNS.map((col) => (
                          <div key={col} className="flex items-center gap-1.5">
                            ...
                            <span style={{ fontSize: "var(--text-caption1)", color: "var(--text-secondary)" }}>
                              {COLUMN_LABELS[col]} ({direct!.byColumn[col]})
                            </span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex gap-3">
                        {COLUMNS.map((col) => (
                          <div key={col} className="flex items-center gap-1.5">
                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: COLUMN_DOT[col] }} />
                            <span style={{ fontSize: "var(--text-caption1)", color: "var(--text-secondary)" }}>
                              {COLUMN_LABELS[col]} ({org.byColumn[col]})
                            </span>
                          </div>
                        ))}
                      </div>
        ...
        {/* ── Skills card ── */}
        {(() => {
          const skills = deriveSkills(agent.tools)
                ...
                {skills.map((skill) => (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${skill.bgColor}`,
                      transition: "all 150ms var(--ease-spring)",
                    }}
                  >
                    <span
