...
import { ChevronRight, RotateCcw, Trash2, Upload, User, X } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const operatorPhotoInputRef = useRef<HTMLInputElement>(null)
  ...
  async function handleOperatorPhotoUpload(file: File) {
    try {
      const dataUrl = await resizeImage(file, 200)
      setOperatorProfileImage(dataUrl)
    } catch {
      // silently fail — user can retry
    }
  }
  ...
  async function handleImageUpload(agentId: string, file: File) {
    try {
      ...
      fetch('/api/agents/update', {
        ...
        body: JSON.stringify({ agentId, profileImage: dataUrl }),
      }).catch(() => {})
    } catch {
      // silently fail — user can retry
    }
  ...
  function syncAgentToOpenClaw(agentId: string, updates: { name?: string; emoji?: string; profileImage?: string | null }) {
    fetch('/api/agents/update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
          ...
          >
            ...
            <div style={{ padding: 'var(--space-3) var(--space-4)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                <button
                  onClick={() => operatorPhotoInputRef.current?.click()}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 'var(--space-2)',
                ...
                <input
                  ref={operatorPhotoInputRef}
                  type="file"
                  accept="image/*"
                  style={{ display: 'none' }}
                  onChange={(e) => {
                    const file = e.target.files?.[0]
                    if (file) handleOperatorPhotoUpload(file)
                    e.target.value = ''
                  }}
                />
                ...
                {settings.operatorProfileImage && (
                  <button
                    onClick={() => setOperatorProfileImage(null)}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val) syncAgentToOpenClaw(agent.id, { emoji: val })
                          }}
                          style={{
                            width: 80,
                            fontSize: 'var(--text-title2)',
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  syncAgentToOpenClaw(agent.id, { profileImage: null })
                                }}
