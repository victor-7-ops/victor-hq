...
/**
 * Read job names from the cron jobs.json config file.
 * This includes deleted/disabled jobs, unlike the CLI list.
 */
export function getJobNames(): Record<string, string> {
  const jobsPath = path.resolve(requireEnv('WORKSPACE_PATH'), '..', 'cron', 'jobs.json')
