...
export function updateCharacter(
  ...
  switch (ch.state) {
    case CharacterState.TYPE: {
      ...
      // If no longer active, just stand idle at current position
      if (!ch.isActive) {
    ...
    case CharacterState.IDLE: {
      ...
      // Inactive agents just stand still — no wandering
      break
    }

    case CharacterState.WALK: {
      // Walk animation (6 frames for MetroCity sprites)
      ...
      if (ch.path.length === 0) {
        ...
        if (ch.isActive) {
          ...
          // Inactive: just stand idle wherever we stopped
          ch.state = CharacterState.IDLE
        }
        ch.frame = 0
        ch.frameTimer = 0
