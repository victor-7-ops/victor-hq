...
import {
  ...
  MC_MATRIX_SPRITE_COLS,
  MC_MATRIX_SPRITE_ROWS,
} from '../../constants'
...
function generateSeeds(): number[] {
  // Generate enough seeds for the larger MetroCity sprites (32 cols)
  const maxCols = Math.max(MATRIX_SPRITE_COLS, MC_MATRIX_SPRITE_COLS)
  const seeds: number[] = []
...
/**
 * Render MetroCity character with Matrix effect.
 * Uses the composited frame's pixel data instead of SpriteData arrays.
 */
export function renderMetroCityMatrixEffect(
  ctx: CanvasRenderingContext2D,
  ch: Character,
  ...
  const isSpawn = ch.matrixEffect === 'spawn'
  const time = ch.matrixEffectTimer
  const spriteCols = MC_MATRIX_SPRITE_COLS
  const spriteRows = MC_MATRIX_SPRITE_ROWS
  const totalSweep = spriteRows + MATRIX_TRAIL_LENGTH

  // Get pixel data from composited frame
  ...
  for (let col = 0; col < spriteCols; col++) {
    const stagger = (ch.matrixEffectSeeds[col % ch.matrixEffectSeeds.length] ?? 0) * MATRIX_COLUMN_STAGGER_RANGE
    const colProgress = Math.max(0, Math.min(1, (progress - stagger) / (1 - MATRIX_COLUMN_STAGGER_RANGE)))
    const headRow = colProgress * totalSweep
      ...
      if (isSpawn) {
        if (distFromHead < 0) continue
          ...
          if (hasPixel) {
            ...
            const a = (1 - trailPos) * MATRIX_TRAIL_EMPTY_ALPHA
            ctx.fillStyle = trailPos < MATRIX_TRAIL_MID_THRESHOLD ? `rgba(0, 255, 65, ${a})`
              : trailPos < MATRIX_TRAIL_DIM_THRESHOLD ? `rgba(0, 170, 40, ${a})`
                : `rgba(0, 85, 20, ${a})`
            ctx.fillRect(px, py, zoom, zoom)
          }
        } else {
