...
import {
  ...
  MC_CHARACTER_HIT_HALF_WIDTH,
  MC_CHARACTER_HIT_HEIGHT,
} from '../../constants'
...
import { pickAppearance } from '../sprites/metrocityLoader'
...
export class OfficeState {
  ...
  addAgent(id: number, preferredPalette?: number, preferredHueShift?: number, preferredSeatId?: string, skipSpawnEffect?: boolean, folderName?: string): void {
    ...
    // Assign MetroCity appearance for visual diversity
    ch.metroCityAppearance = pickAppearance(id)
    if (!skipSpawnEffect) {
  ...
  getCharacterAt(worldX: number, worldY: number): number | null {
    ...
    for (const ch of chars) {
      ...
      // MetroCity sprites are 32x32, anchored bottom-center
      // Apply sitting offset to match visual position
      const sittingOffset = ch.state === CharacterState.TYPE ? MC_CHARACTER_SITTING_OFFSET_PX : 0
      const anchorY = ch.y + sittingOffset
      const left = ch.x - MC_CHARACTER_HIT_HALF_WIDTH
      const right = ch.x + MC_CHARACTER_HIT_HALF_WIDTH
      const top = anchorY - MC_CHARACTER_HIT_HEIGHT
