...
// Generate a deterministic color for any agent name
const PALETTE = [
  "#6366f1", "#f59e0b", "#10b981", "#3b82f6", "#8b5cf6",
  "#f97316", "#14b8a6", "#ec4899", "#ef4444", "#84cc16",
  "#a855f7", "#06b6d4", "#d946ef", "#78716c",
]
...
function getAgentColor(name: string): string {
  if (!AGENT_COLORS[name]) {
    ...
    AGENT_COLORS[name] = PALETTE[Math.abs(hash) % PALETTE.length]
  }
...
function PillTooltip({ slot, rect, containerRect }: { slot: SlotInfo; rect: DOMRect; containerRect: DOMRect }) {
  const color = slot.cron.agentId ? getAgentColor(slot.cron.agentId) : "var(--text-secondary)"
                ...
                return (
                  ...
                  >
                    ...
                    {slots.map((slot, slotIdx) => {
                      ...
                      const color = getAgentColor(agentId) || "var(--text-secondary)"
