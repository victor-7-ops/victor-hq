...
export function OrgMap({ agents, crons, selectedId, onNodeClick }: OrgMapProps) {
  const { nodes: initialNodes, edges: initialEdges } = buildHierarchyLayout(agents, crons, selectedId)
  const [nodes, setNodes, onNodesChange] = useNodesState(initialNodes)
  const [edges, setEdges, onEdgesChange] = useEdgesState(initialEdges)

  useEffect(() => {
    const { nodes: n, edges: e } = buildHierarchyLayout(agents, crons, selectedId)
    setNodes(n)
    setEdges(e)
